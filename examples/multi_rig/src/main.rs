...
use common::{
    add_forest_stack, add_starfield_stack, configure_app, demo_textures, spawn_demo_rig,
...
fn setup(mut commands: Commands, mut images: ResMut<Assets<Image>>) {
    ...
    let forest = spawn_demo_rig(
        &mut commands,
        camera,
        "Forest Rig",
        Vec3::new(0.0, 0.0, 0.0),
    );
    add_forest_stack(&mut commands, forest, &textures);

    let space = spawn_demo_rig(
        &mut commands,
        camera,
        "Space Rig",
        Vec3::new(0.0, 260.0, 0.0),
    );
    add_starfield_stack(&mut commands, space, &textures);
}
